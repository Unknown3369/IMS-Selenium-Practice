...
vendor_master_link = wait.until(
    EC.presence_of_element_located((By.XPATH, "//a[@title='Vendor Master']//span[normalize-space()='Vendor Master']"))
)
driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", vendor_master_link)
time.sleep(2)
driver.execute_script("arguments[0].click();", vendor_master_link)
print("Vendor Master clicked successfully!")
time.sleep(5)

# Click on "Create Vendor" button
add_vendor_button = wait.until(
    EC.element_to_be_clickable((By.XPATH, "//button[@id='create' and normalize-space()='Create Vendor']"))
)
...
# add vendor name
name = wait.until(
    EC.presence_of_element_located((By.XPATH, "//input[@id='vendorName']"))
)
name.send_keys("Vendor from Selenium")
...
# add vendor vat number
vat = wait.until(
    EC.presence_of_element_located((By.XPATH, "//input[@id='vatNo']"))
)
vat.send_keys("123456456")
print("VAT No entered successfully!")

# add vendor contact number
contact_number = wait.until(
    EC.presence_of_element_located((By.XPATH, "//input[@id='Mobile']"))
)
contact_number.send_keys("9844443210")
